...
export default class GenerateCompendiums {
  static async process () {
    ...
    for (const single of batch) {
      ...
      const documents = yamlObject.filter(doc => doc).map(doc => {
        return GenerateCompendiums.processDocument(doc, { type: single.type, pack: single.lang })
      })
            ...
            case 'Actor':
              ...
              if (typeof documents[i].items !== 'undefined') {
                ...
                for (const doc of documents[i].items) {
                  const item = GenerateCompendiums.processDocument(doc, { type: 'Item', pack: single.lang })
                  items.push(item._id)
  ...
  static processDocument (doc, { type = '', lang = 'de' }) {
    if (doc.type === 'folder') {
      if (!doc._id) {
